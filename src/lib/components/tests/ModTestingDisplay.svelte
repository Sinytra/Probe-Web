<script lang="ts">
	...
	async function runTest(id: string): Promise<ModCompatResponse> {
		onstart?.();
		return probeApi.getModCompatibility(id)
			.then(res => {
				onfinish?.();
				return res;
			})
...
<div>
	{#if promise}
		{#await promise}
			<div class="d-flex gap-2 flex-column align-items-center bg-body-tertiary border rounded-1 px-3 py-4 mx-auto">
		...
		{:then result}
			<div class="d-flex gap-2 flex-column bg-body-tertiary border rounded-1 p-3 mx-auto position-relative">
				{#if onclose}
					<Button color="" class="position-absolute text-body-tertiary hover-text-body-secondary"
				...
				<div class="d-flex flex-row gap-3">
					<img class="rounded-1" alt={result.project.slug} src={result.project.icon_url} width="40px" height="40px" />
					<div class="d-flex flex-column text-start text-truncate">
						<span class="fw-bold fs-5 lh-1">{result.project.title}</span>
						<span class="fw-medium fs-7 text-secondary text-truncate">{result.project.description}</span>
					</div>
				</div>
				...
				{#if result.type === 'TESTED'}
					<Container fluid class="mt-1">
						<Row class="fs-6 text-start row-gap-1">
							...
							<Col><span class="fw-medium">Mod Version:</span> <span class="text-nowrap">{result.version_number}</span>
							</Col>
						</Row>
					</Container>
				{/if}

				<div class="my-2 p-2 w-100 text-start px-3 fw-semibold rounded-2
				...
				>
					...
					{:else if result.type === 'NATIVE'}
						<img alt="NeoForge" src={NeoForgeIcon} class="me-1" height="22px" style="margin-bottom: 4px" />
				...
				<div class="d-flex flex-column-reverse gap-3 gap-md-2 flex-md-row justify-content-between">
					{#if result.type === 'TESTED'}
						<Button color="link" class="text-decoration-none serif hover-bg-body-secondary rounded-1 fs-7 px-1
					...
					<div class="ms-md-auto">
						{#if result.type === 'TESTED' && result.passing}
							<a href={modrinth.getVersionURL(result.project.slug, result.version_id)} target="_blank">
						...
						<a href={result.project.url} target="_blank">
							<Button color={ result.type === 'TESTED' && result.passing ? 'secondary' : 'primary' } class="w-md-fit">
								Visit
